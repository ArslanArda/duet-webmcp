...
export function ChordTrack() {
  ...
  const openEditor = (bar: number, symbol: string) => {
    setEditing(bar);
    setValue(symbol);
    setError(false);
  };
  const save = () => {
    if (editing === null || !isValidChord(value)) {
      setError(true);
      return;
    }
    const notes = voiceChord(value, editing).map((draft) => ({
      ...draft,
      id: nanoid(),
      source: "human" as const,
    }));
    setHumanChord(editing, value, notes);
    setEditing(null);
  };
  return (
    <div className="chord-track" style={{ gridTemplateColumns: "72px repeat(16, 112px)" }}>
      ...
      {Array.from({ length: project.barCount }, (_, bar) => {
        ...
        return (
          <div className={`chord-cell ${selected ? "chord-selected" : ""}`} key={bar}>
            {editing === bar ? (
              <form
                onSubmit={(event) => {
              ...
              <button
                onClick={() => {
                  if (editorMode === "erase") {
                    clearRange({ trackId: "chords", startBar: bar, endBar: bar + 1 });
                  } else if (editorMode === "select") {
                    setSelection({ trackId: "chords", startBar: bar, endBar: bar + 1 });
                    completeOnboarding(1);
                  } else openEditor(bar, symbol === "—" ? "" : symbol);
                }}
                title={editorMode === "erase" ? t(locale, "clearBar") : t(locale, "editChord")}
              >
                <span>{bar + 1}</span>
                <b>{symbol}</b>
                <Pencil size={11} />
