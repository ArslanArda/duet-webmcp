...
export function Onboarding() {
  ...
  const copyPrompt = async () => {
    await navigator.clipboard.writeText(t(locale, "promptJazz"));
    ...
    <section className="onboarding" aria-label={t(locale, "startHere")}>
      <div className="onboarding-copy">
        ...
        <button
          ...
          aria-label={t(locale, "skip")}
        >
          <X size={16} />
        </button>
      </div>
      <div className="lesson-row">
        {lessons.map(({ icon: Icon, title, hint, action }, index) => (
          <button
            key={title}
            className={`lesson ${onboarding[index] ? "lesson-complete" : !onboarding.slice(0, index).some((done) => !done) ? "lesson-active" : ""}`}
            onClick={action}
          >
            <span className="lesson-icon">
              {onboarding[index] ? <Check size={15} /> : <Icon size={15} />}
            </span>
            <span>
              <b>
                {index + 1}. {title}
            ...
            {action ? <Copy className="lesson-copy" size={14} /> : null}
          </button>
        ))}
      </div>
    </section>
