...
interface TransportProps { soundReady: boolean; onEnableSound: () => void; onPlay: () => void; onStop: () => void; onRecord: () => void; onLoopChange: (value: boolean) => void; }
export function Transport({ soundReady, onEnableSound, onPlay, onStop, onRecord, onLoopChange }: TransportProps) {
  const { project, locale, changeLog, undoChange, isPlaying, isLooping, isRecording, quantize, setQuantize } = useProjectStore();
  return <footer className="transport-bar">
    ...
    <button className={`transport-button ${isLooping?"transport-active":""}`} onClick={()=>onLoopChange(!isLooping)}>{t(locale,"loop")}</button>
