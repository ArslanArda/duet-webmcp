...
function handleMessage(event: MIDIMessageEvent) {
  ...
  const command = status & 0xf0; const channel = status & 0x0f; const key = `${channel}:${pitch}`;
  if (command === 0x90 && velocity > 0) { held.add(key); callbacks.onNoteOn(pitch, velocity, event.timeStamp, channel); return; }
  if (command === 0xb0 && pitch === 64) {
    const down = velocity >= 64; sustainByChannel.set(channel, down);
    if (!down) [...sustained].filter((item) => item.startsWith(`${channel}:`)).forEach((item) => {
      sustained.delete(item); const note = Number(item.split(":")[1]); callbacks?.onNoteOff(note, event.timeStamp, channel);
    });
    return;
  }
  if (command === 0x80 || (command === 0x90 && velocity === 0)) {
    held.delete(key);
    if (sustainByChannel.get(channel)) sustained.add(key); else callbacks.onNoteOff(pitch, event.timeStamp, channel);
  }
}

export async function connectMidi(nextCallbacks: MidiCallbacks) {
  callbacks = nextCallbacks;
  if (!("requestMIDIAccess" in navigator)) { callbacks.onStatus(null, false); return null; }
  access = await navigator.requestMIDIAccess({ sysex: false });
  access.onstatechange = chooseInput; chooseInput(); return activeInput;
}

export function disconnectMidi() {
  if (activeInput) activeInput.onmidimessage = null;
  if (access) access.onstatechange = null;
  activeInput = null; access = null; callbacks = null; held.clear(); sustained.clear(); sustainByChannel.clear();
}
...
export function parseMidiMessage(data: ArrayLike<number>) {
  ...
  return { command, channel, pitch, velocity, isNoteOn: command === 0x90 && velocity > 0, isNoteOff: command === 0x80 || (command === 0x90 && velocity === 0) };
}
