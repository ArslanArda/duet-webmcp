...
export function generateLine(project: Project, role: LineRole, startBar: number, endBar: number, style: LineStyle = "simple"): NoteDraft[] {
  ...
  for (let bar = startBar; bar < endBar; bar += 1) {
    ...
    if (role === "bass") {
      ...
      beats.forEach((beat, index) => result.push({
        trackId: "bass", pitch: index % 2 === 0 ? root : fifth,
      ...
      const melody = project.notes.filter((note) => note.trackId === "melody" && Math.floor(note.startTick / TICKS_PER_BAR) === bar);
      const melodyAverage = melody.length ? melody.reduce((sum, note) => sum + note.pitch, 0) / melody.length : 69;
      const ordered = [...tones].sort((a, b) => Math.abs(a - (84 - melodyAverage)) - Math.abs(b - (84 - melodyAverage)));
      const beats = style === "syncopated" ? [0.5, 1.5, 2.75] : [0, 2];
      beats.forEach((beat, index) => result.push({
        trackId: "melody", pitch: ordered[index % ordered.length],
        startTick: bar * TICKS_PER_BAR + beat * TICKS_PER_BEAT,
        durationTicks: TICKS_PER_BEAT * (style === "flowing" ? 1.5 : 0.85), velocity: 68,
      }));
    } else {
      const tones = chordPitches(slot.symbol, 52, 76).slice(0, 4);
      const pulse = style === "syncopated" ? [0, 1.5, 3] : style === "flowing" ? [0, 2] : [0];
      pulse.forEach((beat) => tones.forEach((pitch) => result.push({
        trackId: "chords", pitch, startTick: bar * TICKS_PER_BAR + beat * TICKS_PER_BEAT,
        durationTicks: pulse.length === 1 ? TICKS_PER_BAR - 30 : TICKS_PER_BEAT * 1.35, velocity: 58,
      })));
    }
  }
  return result;
