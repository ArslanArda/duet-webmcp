  ...
  persist(
    (set, get) => ({
      ...
      setProjectMeta: (patch) =>
        set((state) => ({ project: { ...state.project, ...patch }, stateVersion: state.stateVersion + 1 })),
      addHumanNote: (input) => {
        const id = nanoid();
        set((state) => ({
      ...
      deleteHumanNote: (id) =>
        set((state) => ({
          project: { ...state.project, notes: state.project.notes.filter((note) => note.id !== id) },
          stateVersion: state.stateVersion + 1,
        })),
      ...
      resetOnboarding: () => set({ onboarding: [false, false, false] }),
